<template>
  <li :class="liClassVal">
    ...
    <a  @mouseenter='enterFunc(model)' @mouseleave='leaveFunc(model)'  @contextmenu.prevent='cxtmenufunc(model)'>
      ...
      <span :class='enableClass' @click='Func(model)' >{{model.name}}</span>
      <!--新增-->
      <span  v-show='model.hover' v-if="model.parent < 3 || model.level_id <= 2" title='新增' class="button add" @click="addNode(model)"></span>
      <!--删除-->
      <span v-show='model.hover' title='删除' class="button remove" @click="delNode(model)"></span>
      <!--启用禁用-->
      <span v-show='model.hover' title='启用/禁用' class="button enable" @click="enable(model)"></span>
      <!--上移-->
  ...
  import Vue from 'vue'
    export default
    {
      ...
      methods:{
        ...
        // 新增节点
        addNode(nodeModel){
          // console.log(nodeModel.level_id);
          if (nodeModel.level_id >= 3){
            console.log("不能继续添加了")
            this.$parent.msg()
            return false
          }
        ...
        // 删除节点
        delNode(nodeModel){
          let data =  JSON.parse(JSON.stringify(nodeModel));
          if(nodeModel) {
            this.$post("/sso/sysfunc/delete",{id:data.id})
              .then(res => {
                if(res.msg == "success"){
        ...
        enable(nodeModel) {
          if(nodeModel) {
            ...
            this.$post("/sso/sysfunc/enable",{id:data.id,status:status})
              .then(res => {
                if(res.msg == "success"){
